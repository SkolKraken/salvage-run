import "./style.css";
import { Game, GRID, FIRE_DAMAGE, ARCHETYPES, ARCHETYPE_ORDER, WEAPON_CATALOG, MISSIONS_PER_RUN, MISSION_DEFS, ENEMY_ARCHETYPES, REPAIR_HP, REPAIR_COST, RECOVER_SALVAGE, RECOVER_CORES, REINFORCE_HP, REINFORCE_SALVAGE, REARM_SALVAGE, REARM_CORES, eq, manhattan, } from "./game";
import { getSprite, spriteDataURL } from "./sprites";
...
const deploySlots = document.getElementById("deploy-slots");
const deployGo = document.getElementById("deploy-go");
const rosterPlayer = document.getElementById("roster-player");
const rosterEnemy = document.getElementById("roster-enemy");
const weaponsEl = document.getElementById("weapons");
const salvageEl = document.getElementById("salvage");
const baySub = document.getElementById("bay-sub");
const baySalvage = document.getElementById("bay-salvage");
const bayCores = document.getElementById("bay-cores");
const bayMechs = document.getElementById("bay-mechs");
const bayRearm = document.getElementById("bay-rearm");
const bayDeploy = document.getElementById("bay-deploy");
const introEl = document.getElementById("mission-intro");
const introKicker = document.getElementById("intro-kicker");
const introTitle = document.getElementById("intro-title");
const introFlavor = document.getElementById("intro-flavor");
const introObjective = document.getElementById("intro-objective");
const introEnemies = document.getElementById("intro-enemies");
const introWaves = document.getElementById("intro-waves");
const introGo = document.getElementById("intro-go");
...
let hover = null;
let shake = 0;
const lance = ["vanguard", "skirmisher", "juggernaut"];
let rearmCtx = null;
let pendingBayFlash = null;
const floaters = [];
...
const MISSION_INTROS = [
    {
        title: "SCRAPFALL",
        flavor: "The crash basin is crawling with scavenger frames. Sweep it clean and stake the claim.",
    },
    {
        title: "CACHE GRAB",
        flavor: "Recon flagged an intact core cache dug into the wreck line upfield. It will be guarded.",
    },
    {
        title: "DUST-OFF",
        flavor: "The salvage barge needs time to spool its lift engines. Hold the field until it screams in.",
    },
];
...
for (let i = 0; i < 2; i++) {
    ...
    btn.addEventListener("click", () => {
        ...
        if (busy || game.phase !== "player" || !u || u.hasFired)
            return;
...
function handleClick(t) {
    ...
    if (u && u.team === "enemy") {
        if (sel && game.canFireWeaponAt(sel, sel.selectedWeapon, u))
            doFire(sel, u);
        return;
    }
    if (sel) {
        ...
        if (crossed) {
            for (const f of crossed)
                spawnFloater(f, `-${FIRE_DAMAGE}`, "#fb923c");
            if (crossed.length) {
                flashUnit(sel.id);
...
function projectileKindFor(w) {
    if (w.range === 1)
        return "slash";
    if (/laser|plasma|beam|railgun/i.test(w.name))
        return "beam";
    return "tracer";
}
function doFire(u, target) {
    ...
    projectiles.push({
        ...
        onLand: () => {
            spawnFloater(targetPos, damage > 0 ? `-${damage}` : "BLOCKED", damage > 0 ? "#fca5a5" : "#94a3b8");
            flashUnit(targetId);
            shake += Math.max(2, damage * 1.5);
            if (coreDropped)
                spawnFloater(targetPos, "+CORE", "#c4b5fd");
            if (targetKilled)
            ...
            refreshUI();
            if (phaseAfter !== "player")
                window.setTimeout(settleBattle, 550);
        },
    });
    refreshUI();
}
            ...
            window.setTimeout(() => {
                ...
                projectiles.push({
                    ...
                    onLand: () => {
                        ...
                        if (hits.length > 0) {
                            for (const h of hits) {
                                spawnFloater(h.pos, h.damage > 0 ? `-${h.damage}` : "BLOCKED", h.damage > 0 ? "#fca5a5" : "#94a3b8");
                                flashUnit(h.id);
                                ...
                                if (h.killed)
                                    spawnExplosion(h.knockTo ?? h.pos);
                            }
                        }
                        else {
...
// --- Salvage Bay ---
function openSalvageBay() {
    rearmCtx = null;
    pendingBayFlash = null;
    renderSalvageBay();
    salvageEl.classList.remove("hidden");
}
function makeBayMech(u) {
    ...
    el.innerHTML =
        `<div class="bay-mech-head"><span>${u.name}</span>` +
            ...
            wpns +
            actions;
    el.querySelectorAll(".bay-wpn-slot").forEach((btn) => {
        btn.disabled = down;
        btn.addEventListener("click", () => {
    ...
    el.querySelectorAll("[data-act]").forEach((btn) => {
        ...
        if (act === "repair")
            btn.disabled = !game.canRepair(u);
        else if (act === "recover")
            btn.disabled = !game.canRecover(u);
        else if (act === "reinforce")
            btn.disabled = !game.canReinforce(u);
        btn.addEventListener("click", () => {
...
function renderRearmPanel() {
    ...
    let html = `<div class="bay-rearm-title">REARM ${u.name} — replace ${u.weapons[slot].name} ` +
    ...
    bayRearm.querySelectorAll("[data-cat]").forEach((btn) => {
...
function renderSalvageBay() {
    ...
    for (const u of game.lance)
        bayMechs.appendChild(makeBayMech(u));
    if (rearmCtx) {
        renderRearmPanel();
    ...
    if (pendingBayFlash) {
        if (pendingBayFlash.salvage)
            flashElement(baySalvage);
        if (pendingBayFlash.cores)
            flashElement(bayCores);
        if (pendingBayFlash.mechId !== null) {
            flashElement(bayMechs.querySelector(`.bay-mech[data-mech-id="${pendingBayFlash.mechId}"]`));
        }
        pendingBayFlash = null;
    }
}
...
function hintText() {
    switch (game.phase) {
        ...
        case "player": {
            ...
            if (!u.hasMoved)
                return `Move ${u.name}, or pick a weapon and target.`;
            return `${u.name} has no clear shot — end turn or switch mech.`;
        }
    }
}
function makeChip(u, pickable) {
    ...
    if (pickable) {
        ...
        el.addEventListener("click", () => {
            if (busy || game.phase !== "player" || u.hp <= 0)
                return;
            game.selectUnit(u.id);
            refreshUI();
        });
    ...
    let html = `<div class="chip-top"><span class="chip-name">${u.name}</span>` +
...
function refreshUI() {
    ...
    for (const u of game.units) {
        if (u.team === "player")
            rosterPlayer.appendChild(makeChip(u, true));
        else
            rosterEnemy.appendChild(makeChip(u, false));
    }
    const sel = game.selected;
    weaponBtns.forEach((btn, i) => {
        ...
        if (sel.hasFired) {
            ...
            .enemies()
            .some((e) => manhattan(sel.pos, e.pos) <= w.range &&
...
function drawTelegraphGround(e) {
    const it = e.intent;
    if (!it)
        return;
    ...
    for (const t of threat) {
        ...
        if (!eq(t, it.attackPos)) {
            ...
            const s = 10;
            ctx.beginPath();
            ctx.moveTo(x + 3, y + 3 + s);
            ctx.lineTo(x + 3, y + 3);
            ctx.lineTo(x + 3 + s, y + 3);
            ctx.moveTo(x + TILE - 3 - s, y + 3);
            ctx.lineTo(x + TILE - 3, y + 3);
            ctx.lineTo(x + TILE - 3, y + 3 + s);
            ctx.moveTo(x + 3, y + TILE - 3 - s);
            ctx.lineTo(x + 3, y + TILE - 3);
            ctx.lineTo(x + 3 + s, y + TILE - 3);
            ctx.moveTo(x + TILE - 3 - s, y + TILE - 3);
            ctx.lineTo(x + TILE - 3, y + TILE - 3);
            ctx.lineTo(x + TILE - 3, y + TILE - 3 - s);
            ctx.stroke();
        }
    }
    if (it.knockbackPos) {
        ...
        ctx.fillRect(it.knockbackPos.x * TILE, it.knockbackPos.y * TILE, TILE, TILE);
        const kFrom = a;
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(kTo.x - kux * ksize + kpx * ksize, kTo.y - kuy * ksize + kpy * ksize);
        ctx.lineTo(kTo.x, kTo.y);
        ctx.lineTo(kTo.x - kux * ksize - kpx * ksize, kTo.y - kuy * ksize - kpy * ksize);
        ctx.stroke();
    }
...
function drawTelegraphMarker(e) {
    const it = e.intent;
    if (!it || !it.attackPos)
        return;
    const a = center(it.attackPos.x, it.attackPos.y);
...
function drawObjectiveMarker() {
    ...
    if (!o.tile)
        return;
    if (game.phase !== "player" && game.phase !== "enemy")
        return;
    ...
    if (o.kind === "extract") {
        ...
        ctx.font = 'bold 8px "Segoe UI", sans-serif';
        ctx.textAlign = "center";
        ctx.fillText(`HOLD ${game.objective.progress}/${game.objective.turnsRequired}`, c.x, o.tile.y * TILE + TILE - 5);
    }
}
...
function drawFireTargets() {
    ...
    for (const e of game.enemies()) {
        if (!game.canFireWeaponAt(u, u.selectedWeapon, e))
            continue;
...
function drawHeatShimmer(u, cx, cy) {
    if (u.team !== "player" || u.heat < u.maxHeat - 1)
        return;
...
function spriteFor(u) {
    if (u.team === "player")
        return (u.archetype ?? "vanguard");
    return (u.enemyKind ?? "stalker");
}
function drawMech(cx, cy, key, facing, alpha, frame) {
    ctx.save();
...
function drawHpPips(cx, cy, u, color) {
    const pipW = 5;
    const gap = 1;
...
function drawUnits() {
    ...
    for (const u of drawable) {
        ...
        const acted = u.team === "player" &&
            ((u.hasMoved && u.hasFired) || u.impaired === "full");
        let alpha = 1;
        if (!alive && dyingRec)
            alpha = Math.max(0, dyingRec.life / dyingRec.maxLife);
        else if (acted)
            alpha = 0.5;
        ...
        if (alive)
            drawHpPips(cx, cy, u, palFor(u).body);
    }
}
